...
if __name__ == '__main__':
    ...
    # 初始化视频捕获设备
    gVideoDevice = cv2.VideoCapture("/home/junwei/KLT-Feature-Tracking/DJI_0012.MP4")
    ...
    cv2.resizeWindow("ROI frame", 1152, 648)
    cv2.moveWindow("ROI frame", 0, 0)
    ...
    # 初始化追踪器
    gTracker = Tracker(tracker_type="BOOSTING")
    ...
    while True:
        ...
        if(gCapStatus):
            ...
            cv2.resizeWindow("track result", 1152, 648)
            cv2.moveWindow("track result", 0, 0)
            cv2.imshow("track result",_item.getFrame())
 
            if _item.getMessage():
                # 打印跟踪数据
                print(_item.getMessage()['coord'][0][0])
            else:
                # 丢失，重新用初始ROI初始
